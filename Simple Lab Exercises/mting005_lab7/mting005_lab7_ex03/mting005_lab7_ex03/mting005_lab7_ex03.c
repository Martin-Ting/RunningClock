...
int displayKPInputSMtick(int state){
	static unsigned char cursorPos = 1;
	...
	switch(state){
		case displayKPInput_display:
			...
			if(KeypadInput!= lastChar && 
			((KeypadInput >= 'A' & KeypadInput <= 'D') || (KeypadInput >= '0' && KeypadInput <= '9') || (KeypadInput == '*' || KeypadInput == '#')) ){
				lastChar = KeypadInput;
